...
def random_resize(image, bboxes):
    ...
    adjusted_bboxes = tf.cond(
        do_a_flip_random < 0.3,
        true_fn=lambda: bbox_resize(image, bboxes, random_diff, random_diff_ratio),
...
def bbox_resize(image, bboxes, random_diff, random_diff_ratio):
    last_coord = get_factor(image)
    ...
    if random_diff_ratio > 0: resized_bboxes = crop_bbox(image, resized_bboxes)
...
def crop_bbox(image, bboxes):
    last_coord = get_factor(image)

    uncropped_bboxes = bboxes[tf.reduce_all(tf.logical_and((bboxes > 0), (bboxes < last_coord)), axis=-1)]
    cropped_bboxes = bboxes[tf.reduce_any(
        tf.logical_and(tf.logical_and((bboxes[:, 0::4] < last_coord), (bboxes[:, 2::4] > 0)),
                       tf.logical_and((bboxes[:, 1::4] < last_coord), (bboxes[:, 3::4] > 0))), axis=-1)]

    xmin, ymin, xmax, ymax = tf.split(value=cropped_bboxes, num_or_size_splits=4, axis=1)
...
def bbox_flip_lr(image, bboxes):
    ...
    flipped_bboxes = tf.pad(flipped_bboxes, [[0, tf.shape(bboxes)[0] - tf.shape(flipped_bboxes)[0]], [0, 0]],
                            'CONSTANT')
